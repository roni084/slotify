...
class AdminEditProfile : AppCompatActivity() {
    ...
    private lateinit var adminAddress: EditText
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateProfile.setOnClickListener { updateProfile(email, businessType, businessName) }
        setupBottomNavigation(email, businessType, businessName)
    }
    ...
    private fun initializeViews(email: String, businessType: String, businessName: String) {
        ...
        adminName = findViewById(R.id.et_name)
        adminEmail = findViewById(R.id.et_email)
        adminPhone = findViewById(R.id.et_phone)
        adminAddress = findViewById(R.id.et_address)
    ...
    private fun loadAdminData(email: String) {
        firestore.collection("Admins").document(email)
            .get()
            .addOnSuccessListener { document ->
                if (document != null && document.exists()) {
                    ...
                    adminName.setText(originalName)
                    adminPhone.setText(originalPhone)
                    adminAddress.setText(originalAddress)
        ...
        MediaManager.get().upload(imagePath)
            ...
            .callback(object : UploadCallback {
                override fun onStart(requestId: String) {
                    // Upload started
                    Toast.makeText(this@AdminEditProfile, "Upload started...", Toast.LENGTH_SHORT).show()
                }
                ...
                override fun onSuccess(requestId: String, resultData: Map<*, *>) {
                    ...
                    val adminRef = firestore.collection("Admins").document(adminEmail.text.toString())
                    adminRef.update("profileImageUrl", imageUrl!!)
                        .addOnSuccessListener {
                            Toast.makeText(this@AdminEditProfile, "Image uploaded and saved!", Toast.LENGTH_SHORT).show()
                        }
                        .addOnFailureListener { e ->
                            Toast.makeText(this@AdminEditProfile, "Failed to save image: ${e.message}", Toast.LENGTH_SHORT).show()
                        }
                ...
                override fun onError(requestId: String, error: ErrorInfo) {
                    // Upload failed
                    Toast.makeText(this@AdminEditProfile, "Upload failed: ${error.description}", Toast.LENGTH_SHORT).show()
                }
    ...
    private fun updateProfile(email: String, businessType: String, businessName: String) {
        val newName = adminName.text.toString().trim()
        val newPhone = adminPhone.text.toString().trim()
        val newAddress = adminAddress.text.toString().trim()
    ...
    private fun setupBottomNavigation(email: String, businessType: String, businessName: String) {
        homeButton.setOnClickListener { navigateTo(AdminHomePage::class.java, email, businessType, businessName) }
        profileButton.setOnClickListener { navigateTo(AdminProfile::class.java, email, businessType, businessName) }
        addButton.setOnClickListener { navigateTo(AddService::class.java, email, businessType, businessName) }
        notificationButton.setOnClickListener { navigateTo(AdminNotification::class.java, email, businessType, businessName) }
        dashboardButton.setOnClickListener { navigateTo(AdminHomePage::class.java, email, businessType, businessName) }
    }

    private fun navigateTo(cls: Class<*>, email: String, businessType: String, businessName: String) {
        startActivity(Intent(this, cls).putExtra("email", email))
        startActivity(Intent(this, cls).putExtra("businessType", businessType))
        startActivity(Intent(this, cls).putExtra("businessName", businessName))
    }
}
